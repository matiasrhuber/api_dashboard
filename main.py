...
def _list_csv_filenames() -> List[str]:
    _ensure_data_dir()
    return sorted([p.name for p in DATA_DIR.iterdir() if p.is_file() and p.suffix.lower() == ALLOWED_SUFFIX])


def _find_cavities_for_prefix(prefix: str) -> List[int]:
    """
    Finds files like '<prefix><n>.csv' where n is int, returns sorted cavity numbers.
    Example prefix: 'hotrunner_cav_temperature_'
    """
    cavities: List[int] = []
    for name in _list_csv_filenames():
        if not name.startswith(prefix) or not name.endswith(".csv"):
            continue
        mid = name[len(prefix):-4]  # strip prefix and ".csv"
        if mid.isdigit():
...
def catalog() -> Dict[str, Any]:
    """
    Lists what the gateway can serve, plus what's actually present in /data.
    """
    available_files = _list_csv_filenames()
...
def oee_catalog() -> Dict[str, Any]:
    return {
        "lines": list(OEE_LINES),
        "metrics": list(OEE_METRICS),
        "files": {
            line: {
                metric: f"oee_{metric}_{line}.csv"
...
def oee_meta(line: str, metric: str) -> Dict[str, Any]:
    ...
    filename = f"oee_{metric}_{line}.csv"
    path = _safe_resolve(filename)
    stat = path.stat()
